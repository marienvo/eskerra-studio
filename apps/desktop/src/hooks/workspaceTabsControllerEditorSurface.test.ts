...
describe('first Today hub after delayed markdown scan', () => {
  let collectCall = 0;

  beforeEach(() => {
    getDesktopMainWindowIntegrationMocks().resetAll();
    collectCall = 0;
    vi.spyOn(eskerraCore, 'collectVaultMarkdownRefs').mockImplementation(async () => {
      collectCall += 1;
      if (collectCall === 1) {
        return [];
      }
      return [{name: 'Today', uri: `${VAULT_ROOT}/Daily/Today.md`}];
    });
  });
  ...
  it('preserves legacy inbox tabs in tabsController when the first hub appears', async () => {
    const NOTE = `${VAULT_ROOT}/Inbox/only.md`;
    const HUB = `${VAULT_ROOT}/Daily/Today.md`;
    const {result, unmount} = await mountHydratedMainWindowWorkspace({
      dirs: [VAULT_ROOT, `${VAULT_ROOT}/Inbox`],
      files: {[NOTE]: 'body\n'},
    });
    ...
    const tabIdBefore = result.current.tabsController.activeEditorTabId;

    await act(async () => {
      result.current.__bumpVaultMarkdownRefsScanForTests?.();
    });

    await waitFor(() => {
      expect(result.current.workspaceShadowModelForTests?.activeHub).toBe(HUB);
      expect(result.current.selectionController.vaultMarkdownRefs.some(r => r.uri === HUB)).toBe(
        true,
      );
    });

    expect(result.current.tabsController.editorWorkspaceTabs).toHaveLength(1);
    expect(result.current.tabsController.editorWorkspaceTabs[0]?.history.entries).toEqual([NOTE]);
    expect(result.current.tabsController.activeEditorTabId).toBe(tabIdBefore);
