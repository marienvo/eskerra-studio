...
| `--color-app-chrome-surface` | `color-mix(in srgb, var(--color-app-chrome-backdrop) 97%, white)` | Almost pure **`palette[0]`** — pressed pane-toggle **icon** (matches chrome bar behind toolbar). |
| `--color-app-chrome-active-fill` | `color-mix(in srgb, var(--color-app-chrome-backdrop) 15%, white)` | Mostly white, hint of theme — pressed pane-toggle **button background** (reads like inactive icon lightness). |
