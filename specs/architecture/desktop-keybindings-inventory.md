...
| `eskerra.vault.compose.saveOrActivateLink` | `Mod-Enter` | Add to inbox compose editor only | Same keymap module with `modEnterSaveWhenNoLink` enabled in [`AddToInboxDialog.tsx`](../../apps/desktop/src/components/AddToInboxDialog.tsx); activates link at caret when applicable, otherwise save (same handler as `Mod-s`) |
