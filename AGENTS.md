...
Canonical agent instructions for this repository (Cursor, Claude Code, Codex, and other AI tools).
...
**Sibling repos:** generic rules and a subset of skills sync to other projects via [`scripts/sync-shared-conventions.sh`](scripts/sync-shared-conventions.sh) — see [`specs/rules/shared-conventions.md`](specs/rules/shared-conventions.md).
...
Applies to: `apps/desktop/src-tauri/src/vault_watch.rs`, `apps/desktop/src/hooks/useWorkspaceVaultWatchEffects.ts`, `apps/desktop/src/hooks/useDiskConflictState.ts`, `apps/desktop/src/hooks/useMergeViewState.ts`, `apps/desktop/src/hooks/useTodayHubsState.ts`, `apps/desktop/src/hooks/useInboxShellRestore.ts`, `apps/desktop/src/hooks/useMainWindowWorkspace.ts`, `apps/desktop/src/hooks/inboxNoteBodyCache.ts`, `apps/desktop/src/lib/vaultFilesChanged*`.

- **Inbox shell restore:** `useInboxShellRestore.ts` applies persisted main-window inbox JSON when it matches the open vault (tabs, active hub, home stacks), then defers one microtask turn to publish React tab state and either `syncShadowWorkspaceFromShellRestore` (non-empty hub projection) or the tab/home shadow mirrors.
- **Today Hub row disk cache:** `todayHubRowLastPersistedRef` in `useTodayHubsState.ts` records the last successfully persisted markdown body per Today row URI (and drops the entry after a successful delete of an all-blank row). `prehydrateTodayHubRows` seeds it together with `inboxContentByUriRef` when a row file exists on disk and the inbox cache does not already hold that URI; `persistTodayHubRow` updates or removes it after successful vault writes. Treat drift between that ref and the inbox cache for the same row URI as a correctness bug on a par with other disk-cache mismatches.
...
- **Single writer:** the daemon (`eskerra-reminderd`) is the **only** process that writes the `@~~…~~` strikethrough mutation. The app **never** writes strike locally — removal from the Notifications pane and **Completed** in the date picker both call `dev.eskerra.Reminders1.RemoveReminder(noteUri, id)` (picker lookup: `reminderTokenLookup.ts`). Unchecking Completed is a local unstrike only. A daemon-unreachable transport error becomes app-local `remove-unavailable` (keep the row, Retry + Open-note) — **never** a local strike write and never recorded as daemon `stale`. Do not add a local-write fallback on any failure path.
